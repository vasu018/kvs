...
void* bstr__alloc (size_t sz) {
	char* p = (char *) rte_malloc (BSTR,sz,0);
	memset (p, 'X', sz);
	return p;
}
#else
#define bstr__alloc(x) rte_malloc (BSTR,x,0)
#endif
#endif

#ifndef bstr__free
#define bstr__free(p) do { rte_free (p);} while(0);
#endif

#ifndef bstr__realloc
#define bstr__realloc(p,x) rte_realloc ((p), (x) ,0)
#endif
